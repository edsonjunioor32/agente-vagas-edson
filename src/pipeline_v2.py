...
import pipeline
from enrichment import enrich_jobs, summarize_enrichment
...
ENRICH_SECOND_LIMIT = pipeline.env_int("ENRICH_SECOND_LIMIT", 60, 0, 250)
...
def write_outputs(jobs, now, enrichment_stats):
    ...
    payload = {"generated_at": now.isoformat(), "count": len(jobs), "enrichment": enrichment_stats, "jobs": jobs}
    (pipeline.OUT / "vagas_ranqueadas.json").write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
    columns = [
        "score", "coverage", "observed_fit", "published_at_br", "title", "company", "work_model", "city", "source",
        "enrichment_status", "enrichment_method", "enrichment_attempts", "description_length", "url", "reasons",
    ]
    with (pipeline.OUT / "vagas_ranqueadas.csv").open("w", encoding="utf-8-sig", newline="") as fp:
        ...
        for job in jobs:
            ...
            writer.writerow(row)
    methods = ", ".join(f"{k}: {v}" for k, v in sorted((enrichment_stats.get("methods") or {}).items())) or "nenhum"
    lines = [
        f"# Vagas priorizadas — {now.strftime('%d/%m/%Y %H:%M')} (UTC-3)", "", f"Total: **{len(jobs)}**",
    ...
    for idx, job in enumerate(jobs[:30], 1):
        ...
        lines += [
            ...
            f"- Motivos: {'; '.join(job.get('reasons') or [])}", f"- Link: {job['url']}", "",
        ]
    (pipeline.OUT / "resumo.md").write_text("\n".join(lines), encoding="utf-8")
    print(f"Pipeline concluído: {len(jobs)} vagas; enriquecimento {enrichment_stats['enriched']}/{enrichment_stats['attempted']}.")
...
def main():
    ...
    # 1ª rodada: melhores candidatas preliminares.
    enrich_jobs(jobs, score_fn=score_job, limit=ENRICH_LIMIT, workers=ENRICH_WORKERS, timeout=ENRICH_TIMEOUT, max_bytes=MAX_HTML_BYTES, max_attempts=1)
    jobs.sort(key=lambda j: (j["score"], j.get("coverage", 0), j["published_at_br"]), reverse=True)

    # 2ª rodada: vagas promovidas pelo primeiro recálculo + uma nova tentativa para falhas transitórias.
    enrich_jobs(jobs, score_fn=score_job, limit=ENRICH_SECOND_LIMIT, workers=ENRICH_WORKERS, timeout=ENRICH_TIMEOUT, max_bytes=MAX_HTML_BYTES, max_attempts=2)
    jobs.sort(key=lambda j: (j["score"], j.get("coverage", 0), j["published_at_br"]), reverse=True)

    write_outputs(jobs, now, summarize_enrichment(jobs))
